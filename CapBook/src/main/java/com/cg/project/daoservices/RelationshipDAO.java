...
public interface RelationshipDAO extends JpaRepository<Relationship, Integer>{
	...
	@Query(value="SELECT * from Relationship r WHERE r.userOneId=:userOneId AND r.userTwoId=:userTwoId",nativeQuery = true)
	...
	@Query(value="SELECT * from Relationship r WHERE r.userOneId=:userOneId OR r.userTwoId=:userTwoId",nativeQuery = true)
	List<Relationship> findAllById(@Param("userOneId") int userOneId);
